...
Infrastructure for autonomous AI agents. Monitor platforms, orchestrate responses, and build digital minds that persist.

## Features

- **Platform Monitoring**: Persistent relay for Twitter DMs, mentions, and WhatsApp messages
- **Session Management**: Per-user and global context scopes with configurable timeouts
- **Auto-Reply**: Multi-model backends (Claude, GPT, custom) with intelligent orchestration
- **Provider System**: Unified interface across Twitter, WhatsApp, and extensible to other platforms
- **Sentient Agents**: Personality system with memory, proactive behavior, and autonomous operation
- **Trading Integration**: Solana memecoin trading capabilities with risk management
...
# Start agent relay
npm run warelay relay --auto-reply
```

## Deploy

- **Railway**: One-click deploy [here](https://railway.app/new/template)
- **Render**: Push to GitHub and connect
- **Vercel**: Website auto-deploys from main branch
- **Manual**: Any Node.js hosting platform
...
```bash
# Link personal WhatsApp Web
warelay login

# Or configure Twilio for WhatsApp
...
```bash
# Start agent relay with auto-reply
warelay relay --auto-reply

# Send messages
warelay send --to +1234567890 --message "Hello"

# Check status
warelay status --verbose
```

## Documentation

- **[WARELAY.md](WARELAY.md)**: Sentient agent setup and configuration
- **[TRADING.md](TRADING.md)**: Trading system setup and strategies  
- **[TERMINAL.md](TERMINAL.md)**: Trading terminal deployment
